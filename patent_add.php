...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($Prof_ID) || empty($Patent_Type) || empty($Patent_Term)) {
        echo "所有欄位都是必填的！";
        exit();
    }

    $stmt = $mysqli->prepare("INSERT INTO Patent (Prof_ID, Patent_Type, Patent_Term) VALUES (?, ?, ?)");
    if ($stmt === false) {
        die("SQL prepare failed: " . htmlspecialchars($mysqli->error));
    }
    $stmt->bind_param("sss", $Prof_ID, $Patent_Type, $Patent_Term);
...
<body>
    ...
    <form method="post">
        ...
        <label>專利名稱/內容：<input type="text" name="Patent_Term" required></label><br>
